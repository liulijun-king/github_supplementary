...
class GitHub(object):
    ...
    def hava_id(self):
        while True:
            if redis_conn.llen(id_key) <= 20000 or redis_conn.llen(day_crawl_key) > 100000:
    ...
    def list_spider(self, project_id):
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info("错误的项目id")
            elif response.status_code == 403 and response.json().get("message") == "Repository access blocked":
                logger.info("无法访问项目id")
            else:
